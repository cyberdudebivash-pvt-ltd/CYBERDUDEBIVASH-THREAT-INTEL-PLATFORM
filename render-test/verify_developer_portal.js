#!/usr/bin/env node
/**
 * SENTINEL APEX — Developer Portal Playwright Verification (PR-8)
 * =================================================================
 * Real-browser (headless Chromium) verification of
 * developer-portal.html, mirroring render-test/verify_enterprise_compliance.js
 * (PR-7). Adds checks specific to this page: keyboard-focusable code
 * blocks, all 3 real data tables (Rate Limits, Error Handling, and the
 * drift-notice comparison table), and that the Documentation
 * Consistency Notice section is actually reachable via its in-page
 * anchor link.
 *
 * Usage:
 *   PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers NODE_PATH="$(npm root -g)" \
 *     node render-test/verify_developer_portal.js
 *
...
const PORT = 8951;
const PAGE_URL = `http://127.0.0.1:${PORT}/developer-portal.html`;
...
async function main() {
  ...
  try {
    ...
    record('All 3 real data tables (Rate Limits, Error Handling, drift-notice comparison) render with rows', tableInfo.count === 3 && tableInfo.rowCounts.every((n) => n > 0), `count=${tableInfo.count} rows=${tableInfo.rowCounts.join(',')}`);
    record('Every table is wrapped for horizontal-scroll safety', tableInfo.allWrapped, `allWrapped=${tableInfo.allWrapped}`);

    const codeBlockInfo = await page1.evaluate(() => {
      const blocks = Array.from(document.querySelectorAll('.pr8-code-block'));
      return {
        count: blocks.length,
        allTabbable: blocks.every((b) => b.getAttribute('tabindex') === '0'),
      };
    });
    record('All code blocks present and keyboard-focusable (tabindex=0)', codeBlockInfo.count >= 3 && codeBlockInfo.allTabbable, `count=${codeBlockInfo.count} allTabbable=${codeBlockInfo.allTabbable}`);

    const firstCodeBlockFocused = await page1.evaluate(() => {
      const el = document.querySelector('.pr8-code-block');
      el.focus();
      return document.activeElement === el;
    });
    record('First code block is programmatically focusable', firstCodeBlockFocused, `focused=${firstCodeBlockFocused}`);

    const driftNoticeReachable = await page1.evaluate(() => {
      const section = document.getElementById('drift-notice');
      const link = document.querySelector('a[href="#drift-notice"]');
      return { sectionExists: !!section, linkExists: !!link };
    });
    record('Documentation Consistency Notice section exists and is linked from an in-page anchor', driftNoticeReachable.sectionExists && driftNoticeReachable.linkExists, JSON.stringify(driftNoticeReachable));

    await page1.click('a[href="#drift-notice"]');
    await page1.waitForTimeout(100);
    const scrolledToNotice = await page1.evaluate(() => {
      const section = document.getElementById('drift-notice');
      const rect = section.getBoundingClientRect();
      return rect.top < window.innerHeight && rect.bottom > 0;
    });
    record('Clicking the drift-notice anchor scrolls it into view', scrolledToNotice, `inViewport=${scrolledToNotice}`);
    ...
    const shotPath = path.join(process.env.PR8_SCREENSHOT_DIR || '/tmp', 'developer-portal-desktop.png');
