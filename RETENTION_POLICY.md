...
| Root-level orphan files (`v55.2-complete.patch`, `SENTINEL_APEX_v78_P0_FIX.tar.gz`, `0001-SENTINEL-APEX-v28.0-FORTRESS-*.patch`, `claude-tasks-history-screenshot.jpg`) | **Move to `archive/root-artifacts-2026-08/` in a single, reversible commit.** Not deleted — zero dependency found, but "not confidently provable as dead" (Phase 1 finding 2) keeps this at archive, not delete. | Immediate, one-time | Phase 1 finding 4, Phase 3 §4 |
| `DASHBOARD-OVERVIEW-LIVE-VIDEO.mp4` | **No retention action — permanent KEEP (updated 2026-09-01).** Removed from the orphan group above: now embedded as the self-hosted preview clip in `index.html`'s `#demo-video` section, with a matching `_headers` cache rule. Live customer-facing production asset. | N/A | Phase 3 §4a |
