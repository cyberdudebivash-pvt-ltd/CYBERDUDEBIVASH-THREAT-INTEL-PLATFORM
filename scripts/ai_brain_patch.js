...
(function(){
  ...
  function buildCampaigns(intel){
    ...
    intel.forEach(function(t,idx){
      ...
      var rawActor=t.primary_actor||t.actor_display_name||t.mitre_group_name||t.actor||'';
      ...
      (t.mitre_tactics||t.ttps||[]).forEach(function(tt){if(tt&&tt.tactic)c.tactics.add(tt.tactic);});
  ...
  function buildAnomalies(intel){
    ...
    /* Real feed schema exposes risk_score / cvss_score / kev_present /
       ...
       live item. */
    var scores=intel.map(function(t){return parseFloat(t.risk_score||t.cvss_score||5);});
    ...
    var anomalies=intel.map(function(t,idx){
      var score=parseFloat(t.risk_score||t.cvss_score||5);
  ...
  function buildSOCQueue(intel){
    return intel
      ...
      .map(function(t){
        ...
        var score=parseFloat(t.risk_score||t.cvss_score||0);
