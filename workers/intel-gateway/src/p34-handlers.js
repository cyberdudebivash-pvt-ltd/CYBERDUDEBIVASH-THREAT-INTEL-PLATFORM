...
export async function handleP34Performance(request, env) {
  ...
  // Measure KV round-trip latency. PRODUCTION-VERIFICATION FIX
  // (2026-08-24): THREAT_INTEL_KV is not a bound namespace (see this
  // file's _loadFeed fix note); this read had no try/catch, so every
  // call to this endpoint crashed to a 500. Measured against
  // SECURITY_HUB_KV (a real, always-bound namespace) instead -- any
  // bound KV namespace gives an equally representative round-trip
  // latency figure for this diagnostic.
  await env.SECURITY_HUB_KV.get('feed:latest').catch(() => null);
...
export async function handleP34Sbom(request, env) {
  // Surfaces SBOM status from R2 if present, otherwise returns structural declaration.
  // SECURITY (PR #407, CodeRabbit CWE-200): this route is intentionally unauthenticated
  // -- it is the live data source for the public enterprise-assurance-center.html trust
  // center -- so the full report (package list, file hashes/paths) is deliberately never
  // returned here, only the status/format fields the trust-center UI actually renders.
  // See CHANGELOG.md [200.0.0-rc] for the prior audit of this route's auth posture.
...
export async function handleP34Contracts(request, env) {
  // SECURITY (PR #407, CodeRabbit CWE-200): same public/unauthenticated trust-center
  // route as handleP34Sbom above -- return status + non-sensitive counts only, never
  // the raw report (it carries internal schema field names and per-entry violation
  // detail, e.g. data/governance/contract_drift_report.json's `violations`/
  // `baseline_drift` arrays).
  ...
  return _json({
    ...
    breaking_change_count: typeof driftReport?.breaking_changes === 'number' ? driftReport.breaking_changes : null,
    entries_checked: typeof driftReport?.entries_checked === 'number' ? driftReport.entries_checked : null,
