...
const RX_PUB_A0_VERSION = 'a0.5';
const MANIFEST_KEY = 'intel/rx_pub_a0_reports_artifact_manifest.json';

// RX-PUB-A0.5 Section 16: a stale manifest must never read as a current
// "healthy" summary. sentinel-blogger.yml's own schedule is 3x/day (every
// 8h) plus push-triggered runs; 16h (same margin P40's own
// CERT_FRESHNESS_STALE_SECONDS already uses for a report generated by this
// same pipeline) tolerates one missed scheduled run without a false STALE,
// while still catching a genuinely stuck/broken pipeline.
const FRESHNESS_STALE_SECONDS = 16 * 3600;
...
export async function handleRxPubA0ReportsIdentity(request, env) {
  ...
  const ageSeconds = Number.isFinite(generatedAtMs)
    ...
    : null;
  // RX-PUB-A0.5 Section 16: never let a stale manifest silently read as
  // current-healthy. UNKNOWN (unparseable/missing generated_at) is treated
  // as not-fresh too -- an unknown age is not evidence of freshness.
  const status = ageSeconds === null
    ? 'UNKNOWN'
    : (ageSeconds > FRESHNESS_STALE_SECONDS ? 'STALE' : 'HEALTHY');
