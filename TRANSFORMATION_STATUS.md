...
- Retirement of Railway-specific configuration (`railway.json`, `Dockerfile.railway`, `Procfile`, `sentinel-apex-api/`, `agent/api/api_server.py`, `api/main.py`, root `Dockerfile`) is gated on confirming whether any currently-served page still depends on a Railway-hosted endpoint (still unconfirmed), **and now also** on updating `enterprise-rollback-governance.yml`'s critical-file list and `scripts/soc2_compliance_engine.py`'s compliance checks (found Batch 6).
...
- `sentinel-apex-api/`'s retirement readiness is `NEEDS VALIDATION`, not `READY` or `BLOCKED` — several whole-repo maintenance scripts reference it, most likely as generic repo-wide iteration rather than a real dependency, but this wasn't individually confirmed for each script (Batch 6).
...
- Executing the 4 `READY` retirements once explicitly approved (Batch 6 only assessed readiness; nothing was removed this phase).
