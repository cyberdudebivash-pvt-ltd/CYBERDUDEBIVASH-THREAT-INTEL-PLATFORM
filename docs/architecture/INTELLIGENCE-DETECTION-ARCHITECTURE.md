...
**CORRECTION (2026-08-31), verified against current `main`:** §13's "central finding" is stale. A Phase 4.1 pass (same commit as this document, `043e53633`) fixed the live-endpoint disconnection directly in the actual production surface — `workers/intel-gateway/src/detection-registry.js`, wired into `workers/intel-gateway/src/index.js`'s `GET /api/v1/detections` — but never updated this document's prose to match. Worse, that fix's own header comment records a sharper root cause than §13 states: `api/v1_router.py`/`api/engine_connector.py` (the files §13 analyzes) were **never mounted by any running server** — `Dockerfile.api`'s CMD references a module path that doesn't exist in this repo, and the file is referenced only by `sast-security-scan.yml` (a security-scan build, not a deployment). §13's "the platform's own public detections API endpoint is currently serving empty or default data" was accurate about `api/engine_connector.py` in isolation, but that module was never the live endpoint to begin with — confirmed directly against `intel.cyberdudebivash.com` this session. The live `/api/v1/detections` today correctly sources from generator C (`detection_bundle_injector.py`), with real tiering, pagination, structural validation, and fail-closed error handling. See `detection-registry.js`'s own header for the full trace. §16 finding #1 and §17's finding-1 recommendation are superseded by this note. **Update:** findings #2 and #4 were also found stale/already-fixed on re-check the same day (see §16) -- three of four findings checked so far turned out to already be resolved, with this table simply never updated to match. Treat every row in §16 as needing a fresh check against current code before acting on it, not as a reliable live status.
...
|---|---|---|---|
| 1 | `/api/v1/detections` reads from a non-existent, never-written data source, disconnected from all 4 real generators | **P0 — live customer-facing defect** | **Superseded — see the 2026-08-31 correction note at the top of this document. `api/engine_connector.py` was never the live endpoint; the actual live route was fixed the same commit via `detection-registry.js`.** |
| 2 | Generator C has no per-item exception isolation and an all-or-nothing write; a mid-loop failure silently discards the entire run's progress, masked by workflow-level `continue-on-error` | P1 | **Stale as of 2026-08-31 — already fixed.** `scripts/detection_bundle_injector.py`'s main loop (confirmed by direct reading) now wraps each item's processing in its own `try`/`except`, logs the failure, and `continue`s -- one bad item costs exactly that item, not the run. The fix is already in-code with its own inline comment citing "Phase 4.1 mandate Section 20" -- this table just never got updated to match. |
| 3 | Only generator C applies eligibility gating; A/B/D generate unconditionally, including D producing full 6-format packs for NEWS items | P1 | No -- generator A side re-confirmed 2026-08-31 (`stage_3_1_11_detection_engineering_core.py`'s `load_advisories()` and batch loop apply no eligibility filter); B/D not re-checked this pass. |
| 4 | `is_detection_eligible()` checks `cve_id`/`cve_ids` but not the third field-name variant `cves` | P2 (same class as `INTELLIGENCE-REPORT-QUALITY.md` §7) | **Stale as of 2026-08-31 — already fixed.** `p38_shared_validators.get_cve_ids()` (confirmed by direct reading) merges `cve_ids`, `cves`, and `cve_id` with dedup, and `is_detection_eligible()` calls it directly -- all three variants are covered. |
...
Findings 1–3 are architecturally significant enough to warrant their own dedicated investigation-and-fix pass rather than a patch appended to this already-large quality-architecture PR: ~~finding 1 requires deciding whether to point `engine_connector.py` at one of the four real generators' output...~~ **(finding 1 is resolved — see the 2026-08-31 correction note at the top of this document; no further action needed there)**; finding 2 requires adding per-item isolation to `detection_bundle_injector.py`'s main loop plus a partial-write strategy, which touches the same file this document's Checkpoint E already modified once this phase — a second surgical pass, not a bundled one, keeps that change auditable. Recommended as explicit, prioritized Phase 5 scope rather than rushed into Phase 4's remaining budget.

**2026-08-31 addendum — a further, related finding, not in the original Phase 4 scope:** generator A's pipeline (`sentinel-blogger.yml`'s STAGE 3.1.11, `scripts/stage_3_1_11_detection_engineering_core.py`) is actively scheduled (4x/day cron in `generate-and-sync.yml`) and does run, but its output (`api/detections/detection-index.json`, `data/audit/stage_3_1_11_detection_engineering.json`) has been stuck at exactly 11 advisories since 2026-05-25 — confirmed via matching counts in both independent files despite 3+ months of scheduled runs since. This is a live stall in an active pipeline stage, not a wiring gap like finding 1 — worth its own root-cause investigation given generator A produces the richest output of all four generators (real MITRE technique IDs, 14 SIEM/EDR export formats) if unstuck. Not investigated further in this pass.
