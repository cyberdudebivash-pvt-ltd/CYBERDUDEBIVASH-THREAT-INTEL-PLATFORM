...
| `r2_reports_verifier.py` | `sentinel-apex-reports` | Bounded (~500-item in-window scope, per its own docstring) | B | Unchanged — already bounded, not a contributor. |
| `r2_upload_verifier.py` | `sentinel-apex-data` | Single-object HEAD check | B | Unchanged. |
| `r2_state_sync.py` | `sentinel-apex-data` | ~9 small state files + 1 bounded dir sync, called 3×/run | A/B | Unchanged — small, already bounded, pre-justified duplicate call sites (documented pipeline-ordering reasons). Flagged as a residual low-priority item, not fixed in this PR (see §7). |
| `r2_resync_manifests.py` | `sentinel-apex-data` | ~51 items, called 2×/run | A | Unchanged — same rationale as above. |
...
- `r2_state_sync.py` and `r2_resync_manifests.py` are each invoked twice per pipeline run. Both duplications are pre-existing, individually documented, deliberate fixes for real pipeline-ordering bugs (a manifest changes between the two call sites in each case). Small, already-bounded (~9 and ~51 objects respectively), not a contributor to this incident — left unchanged per Zero Unnecessary Modification. Flagged here for visibility, not fixed.
- `r2_reports_integrity.py` / `r2_reports_verifier.py` remain bounded-but-nonzero Class B consumers every run (≤200 and ~500 objects respectively). Already far below any dangerous threshold and not part of the root cause — left unchanged.
